...
dtrain <- xgb.DMatrix(data = data.matrix(train), label=out1)
dtest  <- xgb.DMatrix(data = data.matrix(test),  label=out2)
...
eval(x=outcome,type='brier.m')  # 0.190264
eval(x=outcome,type='brier.f')  # 0.1577465
eval(x=outcome,type='brier.a')  # 0.1862798
eval(x=outcome,type='fair.m')   # 0.8026331
eval(x=outcome,type='fair.f')   # 0.8258991
auc_roc(preds = outcome$pred,actuals = outcome$out) #0.699691
...
# colsample_bytree = 0.45, optimized
...
eval(x=outcome,type='brier.m')  # 0.1888102
eval(x=outcome,type='brier.f')  # 0.1552576
eval(x=outcome,type='brier.a')  # 0.1846992
eval(x=outcome,type='fair.m')   # 0.8071512
eval(x=outcome,type='fair.f')   # 0.8244481
auc_roc(preds = outcome$pred,actuals = outcome$out) #0.7057917
...
dtest2  <- xgb.DMatrix(data = data.matrix(final_test[,c(1,ind)]))
...
write.csv(outcome2, 
          here('data','comptetition_test_model3.csv'),
          row.names = FALSE)
