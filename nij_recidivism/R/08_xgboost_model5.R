...
# The most important 50 predictor from the prior model (Model 4)
ind <-  c("comp1", "age1c", "gang", "prop", "risk1", "felony", "Xv1", "gender", 
          "age1", "age7", "year3", "year1c", "mhsa", "off3", "comp3", "race",
          "coged","prior_parole","cmisd","misd","viol","comp2","off5","off1",
          "drug","educ3","agri14","educ2","cprop","hins7","cviol","slevel3",
          "slevel2","age3","cother","prior_prob","gcm1","hospital17","off2",
          "age5c","off4","educ1","age2c","transfer2","vital11","slevel1",
          "vital3","year2","assault","dep")
...
test  <- train_[loc2,c("ID",ind)]
train <- train_[loc1,c("ID",ind)]
...
grid <- expand.grid(etas = seq(0.005,.15,.001))
...
dtest2  <- xgb.DMatrix(data = data.matrix(final_test[,ind]))
...
write.csv(outcome2, 
          here('data','comptetition_test_model5.csv'),
          row.names = FALSE)
