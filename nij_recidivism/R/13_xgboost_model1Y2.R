...
eval(x=outcome,type='brier.m')  
eval(x=outcome,type='brier.f')  
eval(x=outcome,type='brier.a')  # 0.1656638
eval(x=outcome,type='fair.m')   
eval(x=outcome,type='fair.f')   
auc_roc(preds = outcome$pred,actuals = outcome$out) #0.7222203
...
write.csv(outcome2, 
          here('data','comptetition_test_model1_Y2.csv'),
          row.names = FALSE)
