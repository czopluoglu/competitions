...
test_submit <- read.csv(here('data','NIJ_s_Recidivism_Challenge_Test_Dataset3.csv'))
...
table(full_data$Recidivism_Arrest_Year2,
      full_data$Recidivism_Arrest_Year3)

full_data <- full_data[full_data$Recidivism_Arrest_Year1=='false',]
...
# Group 1: felony, misd, prop, cfelony, cmisd, cprop
# Group 2: drug, cdrug
# Group 3: viol, dv, cviol
...
full_data$comp3 <- rowMeans(full_data[,c('viol','dv')],na.rm=TRUE)
...
test_submit$comp3 <- rowMeans(test_submit[,c('viol','dv')],na.rm=TRUE)


###############################################################################

final_test <- test_submit[,c(1,50:168)]
...
save.image("B:/Ongoing_Research/nij/nij/data/final_dataY3.RData")
